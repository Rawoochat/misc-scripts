...
#
# convert-to-opus.sh
# Converts .mp3, .ogg, .m4a, .flac files to .opus while preserving
# metadata AND album art (even from MP3 ID3 APIC tags).
#
# Usage:  ./convert-to-opus.sh /path/to/music [bitrate]
#         bitrate is optional, default 128k (very good quality for Opus)
#
# Requirements: ffmpeg (with libopus), python3, find, coreutils
#
...
#
# Opus-in-OGG does NOT support video streams — it uses a Vorbis
# comment called METADATA_BLOCK_PICTURE (inherited from the FLAC
# spec).  ffmpeg's OGG/Opus muxer does NOT write that comment
...
#   3. Pass the result as a Vorbis metadata tag during the Opus encode.
#
...
if [[ $# -lt 1 ]]; then
    ...
    echo "  bitrate       Opus bitrate (default: 128k). Good values: 64k–192k"
    exit 1
fi
...
#
# The Opus/OGG container embeds cover art as a Vorbis comment whose
...
# ─────────────────────────────────────────────────────────────────────
# Convert a single file to Opus with metadata + album art
# ─────────────────────────────────────────────────────────────────────
convert_file() {
    local src="$1"
    local dst="$2"
...
for filepath in "${FILES[@]}"; do
    ...
    out_path="${OUTPUT_DIR}/${relative%.*}.opus"
...
if [[ $converted -gt 0 ]] && command -v du &>/dev/null; then
    ...
    opus_size=$(find "$OUTPUT_DIR" -iname '*.opus' -type f \
        -exec du -cb {} + 2>/dev/null | tail -1 | cut -f1)
    if [[ -n "${opus_size:-}" && "${orig_size:-0}" -gt 0 ]]; then
        ratio=$(awk "BEGIN { printf \"%.1f\", ($opus_size / $orig_size) * 100 }")
        orig_h=$(numfmt --to=iec-i --suffix=B "$orig_size" 2>/dev/null || echo "${orig_size} bytes")
        opus_h=$(numfmt --to=iec-i --suffix=B "$opus_size" 2>/dev/null || echo "${opus_size} bytes")
        echo ""
        echo -e "  ${BOLD}Size comparison:${RESET}"
        echo -e "    Originals:  $orig_h"
        echo -e "    Opus files: $opus_h  (${ratio}% of original)"
    fi
fi
